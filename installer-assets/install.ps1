...
$UseCnMirrors = $env:OPENCLAW_USE_CN_MIRRORS -eq "1" -or $env:OPENCLAW_MIRROR_PRESET -eq "cn"
$InstallBaseUrl = if (-not [string]::IsNullOrWhiteSpace($env:OPENCLAW_INSTALL_BASE_URL)) {
    $env:OPENCLAW_INSTALL_BASE_URL.TrimEnd("/")
} elseif ($UseCnMirrors) {
    "https://clawd.org.cn"
} else {
    "https://openclaw.ai"
}
$NpmRegistry = if (-not [string]::IsNullOrWhiteSpace($env:OPENCLAW_NPM_REGISTRY)) {
    $env:OPENCLAW_NPM_REGISTRY.TrimEnd("/")
} elseif ($UseCnMirrors) {
    "https://registry.npmmirror.com"
} else {
    ""
}
$OpenClawRepoUrl = if (-not [string]::IsNullOrWhiteSpace($env:OPENCLAW_GIT_REPO_URL)) {
...
function Get-InstallScriptUrl {
    ...
    return "$InstallBaseUrl/$Name"
}

function Invoke-WithNpmMirror {
    param(
        [Parameter(Mandatory = $true)]
        [scriptblock]$Script
    )
    ...
    $prevNpmRegistryLower = $env:npm_config_registry
    $prevCorepackRegistry = $env:COREPACK_NPM_REGISTRY
    ...
    try {
        ...
        $env:npm_config_registry = $prevNpmRegistryLower
        $env:COREPACK_NPM_REGISTRY = $prevCorepackRegistry
    }
}
...
function Resolve-CommandPath {
        ...
        $preferredCommands = @(
            $commands |
                Sort-Object @{
                    Expression = {
                        $path = $_.Source
                        if ([string]::IsNullOrWhiteSpace($path)) { $path = $_.Path }
                        if ([string]::IsNullOrWhiteSpace($path)) { $path = $_.Definition }
                        if ($_.CommandType -eq "Application" -and $path -match "\.(cmd|exe|bat|com)$") { return 0 }
                        if ($_.CommandType -eq "Application") { return 1 }
                        return 2
                    }
                }
        )
    ...
    foreach ($command in $preferredCommands) {
        ...
        if ($command -and -not [string]::IsNullOrWhiteSpace($command.Path) -and (Test-Path $command.Path)) {
            return (Resolve-Path $command.Path).Path
        }
        if ($command -and -not [string]::IsNullOrWhiteSpace($command.Definition) -and (Test-Path $command.Definition)) {
            return (Resolve-Path $command.Definition).Path
        }
    }
...
function Invoke-StreamingProcess {
    ...
    $resolvedFilePath = Resolve-CommandPath -CommandName $FilePath

    $startInfo = New-Object System.Diagnostics.ProcessStartInfo
    $startInfo.FileName = $resolvedFilePath
    # Use .Arguments (single string) instead of .ArgumentList (PS7/.NET Core only)
    # to ensure compatibility with both PowerShell 5 and 7.
    ...
    # Use a synchronous reader pattern that works reliably on both PS5 and PS7.
    # We read stdout and stderr synchronously in the background using .NET threads.
    $outputLines = New-Object System.Collections.Concurrent.ConcurrentBag[string]
    ...
    $stdoutThread = [System.Threading.Tasks.Task]::Run([System.Action]{
        while ($true) {
    ...
    $stderrThread = [System.Threading.Tasks.Task]::Run([System.Action]{
        while ($true) {
    ...
    # Wait for all streams and the process to finish
    $process.WaitForExit()
    try { $stdoutThread.Wait(5000) | Out-Null } catch {}
    try { $stderrThread.Wait(5000) | Out-Null } catch {}
...
function Install-OpenClaw {
    ...
    $env:NPM_CONFIG_SCRIPT_SHELL = "cmd.exe"
    try {
        $npmResult = Invoke-WithNpmMirror {
            Invoke-StreamingProcess -FilePath "npm" -Arguments @("install", "-g", "$packageName@$Tag") -HeartbeatMessage "[*] OpenClaw npm install is still running..." -HeartbeatSeconds 12
        }
        ...
        if ($npmResult.ExitCode -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host "  iwr -useb $(Get-InstallScriptUrl "install.ps1") | iex" -ForegroundColor Cyan
            }
