...
set "TAG_SET=0"
set "INSTALL_PS1_URL="
set "INSTALL_PS1_URL_EXPLICIT=0"
...
set "INSTALL_BASE_URL=https://openclaw.ai"
if /i "%OPENCLAW_USE_CN_MIRRORS%"=="1" set "INSTALL_BASE_URL=https://clawd.org.cn"
if /i "%OPENCLAW_MIRROR_PRESET%"=="cn" set "INSTALL_BASE_URL=https://clawd.org.cn"
if not "%OPENCLAW_INSTALL_BASE_URL%"=="" set "INSTALL_BASE_URL=%OPENCLAW_INSTALL_BASE_URL%"
if not "%OPENCLAW_INSTALL_PS1_URL%"=="" (
  set "INSTALL_PS1_URL=%OPENCLAW_INSTALL_PS1_URL%"
  set "INSTALL_PS1_URL_EXPLICIT=1"
)
if "%INSTALL_PS1_URL%"=="" set "INSTALL_PS1_URL=%INSTALL_BASE_URL%/install.ps1"
set "FALLBACK_INSTALL_PS1_URL=https://openclaw.ai/install.ps1"

if exist "%INSTALL_PS1_URL%" (
  copy /Y "%INSTALL_PS1_URL%" "%TMP%" >nul
) else (
  curl -fsSL "%INSTALL_PS1_URL%" -o "%TMP%"
  if %ERRORLEVEL% neq 0 if "%INSTALL_PS1_URL_EXPLICIT%"=="0" if /i not "%INSTALL_PS1_URL%"=="%FALLBACK_INSTALL_PS1_URL%" (
    echo Mirror download failed, retrying official origin... >&2
    curl -fsSL "%FALLBACK_INSTALL_PS1_URL%" -o "%TMP%"
  )
)
if %ERRORLEVEL% neq 0 (
