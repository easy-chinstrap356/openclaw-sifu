...
OPENCLAW_MIRROR_PRESET="${OPENCLAW_MIRROR_PRESET:-}"
OPENCLAW_USE_CN_MIRRORS="${OPENCLAW_USE_CN_MIRRORS:-0}"
OPENCLAW_INSTALL_BASE_URL="${OPENCLAW_INSTALL_BASE_URL:-}"
OPENCLAW_NPM_REGISTRY="${OPENCLAW_NPM_REGISTRY:-}"
OPENCLAW_GITHUB_PROXY="${OPENCLAW_GITHUB_PROXY:-}"
OPENCLAW_GIT_REPO_URL="${OPENCLAW_GIT_REPO_URL:-}"
OPENCLAW_GUM_BASE_URL="${OPENCLAW_GUM_BASE_URL:-}"
OPENCLAW_HOMEBREW_INSTALL_URL="${OPENCLAW_HOMEBREW_INSTALL_URL:-}"
OPENCLAW_NODESOURCE_DEB_URL="${OPENCLAW_NODESOURCE_DEB_URL:-}"
OPENCLAW_NODESOURCE_RPM_URL="${OPENCLAW_NODESOURCE_RPM_URL:-}"

ORIGINAL_PATH="${PATH:-}"

mirror_preset_cn_enabled() {
    [[ "$OPENCLAW_USE_CN_MIRRORS" == "1" || "$OPENCLAW_MIRROR_PRESET" == "cn" ]]
}
...
apply_mirror_defaults() {
    if mirror_preset_cn_enabled; then
        if [[ -z "$OPENCLAW_INSTALL_BASE_URL" ]]; then
            OPENCLAW_INSTALL_BASE_URL="https://clawd.org.cn"
        fi
        if [[ -z "$OPENCLAW_NPM_REGISTRY" ]]; then
            OPENCLAW_NPM_REGISTRY="https://registry.npmmirror.com"
        fi
    fi

    OPENCLAW_INSTALL_BASE_URL="$(normalize_base_url "$OPENCLAW_INSTALL_BASE_URL")"
    OPENCLAW_NPM_REGISTRY="$(normalize_base_url "$OPENCLAW_NPM_REGISTRY")"
    OPENCLAW_GITHUB_PROXY="$(normalize_base_url "$OPENCLAW_GITHUB_PROXY")"
    OPENCLAW_GUM_BASE_URL="$(normalize_base_url "$OPENCLAW_GUM_BASE_URL")"
    OPENCLAW_HOMEBREW_INSTALL_URL="$(normalize_base_url "$OPENCLAW_HOMEBREW_INSTALL_URL")"
}

installer_origin_base_url() {
    echo "https://openclaw.ai"
}

installer_primary_base_url() {
    if [[ -n "$OPENCLAW_INSTALL_BASE_URL" ]]; then
        echo "$OPENCLAW_INSTALL_BASE_URL"
        return 0
    fi
    installer_origin_base_url
}
...
installer_script_url() {
    local name="$1"
    echo "$(installer_primary_base_url)/${name}"
}

openclaw_repo_url() {
    if [[ -n "$OPENCLAW_GIT_REPO_URL" ]]; then
        echo "$OPENCLAW_GIT_REPO_URL"
        return 0
    fi
    echo "https://github.com/openclaw/openclaw.git"
}
...
add_unique_candidate() {
    ...
    for existing in "${DOWNLOAD_CANDIDATES[@]:-}"; do
...
build_download_candidates() {
    ...
    case "$url" in
        https://openclaw.ai/*)
            if [[ -n "$OPENCLAW_INSTALL_BASE_URL" ]]; then
                add_unique_candidate "${OPENCLAW_INSTALL_BASE_URL}${url#https://openclaw.ai}"
            fi
            ;;
    esac

    if [[ -n "$OPENCLAW_GITHUB_PROXY" ]]; then
        case "$url" in
            https://github.com/*|https://raw.githubusercontent.com/*)
                add_unique_candidate "$(prefix_url "$OPENCLAW_GITHUB_PROXY" "$url")"
                ;;
        esac
    fi

    case "$url" in
        https://github.com/charmbracelet/gum/releases/download/*)
            if [[ -n "$OPENCLAW_GUM_BASE_URL" ]]; then
                add_unique_candidate "${OPENCLAW_GUM_BASE_URL}${url#https://github.com/charmbracelet/gum/releases/download}"
            fi
            ;;
        https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)
            if [[ -n "$OPENCLAW_HOMEBREW_INSTALL_URL" ]]; then
                add_unique_candidate "$OPENCLAW_HOMEBREW_INSTALL_URL"
            fi
            ;;
        https://deb.nodesource.com/setup_22.x)
            if [[ -n "$OPENCLAW_NODESOURCE_DEB_URL" ]]; then
                add_unique_candidate "$OPENCLAW_NODESOURCE_DEB_URL"
            fi
            ;;
        https://rpm.nodesource.com/setup_22.x)
            if [[ -n "$OPENCLAW_NODESOURCE_RPM_URL" ]]; then
                add_unique_candidate "$OPENCLAW_NODESOURCE_RPM_URL"
            fi
            ;;
    esac

    add_unique_candidate "$url"
}

with_npm_registry_env() {
    if [[ -z "$OPENCLAW_NPM_REGISTRY" ]]; then
        "$@"
        return $?
    fi
    ...
    env \
        ...
        "npm_config_registry=$OPENCLAW_NPM_REGISTRY" \
        "COREPACK_NPM_REGISTRY=$OPENCLAW_NPM_REGISTRY" \
        "$@"
}
...
download_file() {
    ...
    build_download_candidates "$url"
    for candidate in "${DOWNLOAD_CANDIDATES[@]}"; do
        if [[ "$DOWNLOADER" == "curl" ]]; then
            if curl -fsSL --proto '=https' --tlsv1.2 --retry 3 --retry-delay 1 --retry-connrefused -o "$output" "$candidate"; then
                return 0
            fi
        else
            if wget -q --https-only --secure-protocol=TLSv1_2 --tries=3 --timeout=20 -O "$output" "$candidate"; then
                return 0
            fi
        fi
    done
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb $(installer_script_url install.ps1) | iex"
        exit 1
    fi
...
Usage:
  curl -fsSL --proto '=https' --tlsv1.2 $(installer_script_url install.sh) | bash -s -- [options]
...
Environment variables:
  ...
  OPENCLAW_USE_CN_MIRRORS=1            Prefer CN mirrors for OpenClaw script hosting and npm
  OPENCLAW_MIRROR_PRESET=cn            Same as OPENCLAW_USE_CN_MIRRORS=1
  OPENCLAW_INSTALL_BASE_URL=...        Override install.sh/install.ps1 host (e.g. https://clawd.org.cn)
  OPENCLAW_NPM_REGISTRY=...            Override npm/pnpm registry
  OPENCLAW_GIT_REPO_URL=...            Override git clone URL for OpenClaw
  OPENCLAW_GITHUB_PROXY=...            Prefix GitHub/raw.githubusercontent.com downloads, then fall back
  OPENCLAW_GUM_BASE_URL=...            Override gum release base URL
  OPENCLAW_HOMEBREW_INSTALL_URL=...    Override Homebrew install script URL
  OPENCLAW_NODESOURCE_DEB_URL=...      Override NodeSource deb setup script URL
  OPENCLAW_NODESOURCE_RPM_URL=...      Override NodeSource rpm setup script URL
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 $(installer_script_url install.sh) | bash
  curl -fsSL --proto '=https' --tlsv1.2 $(installer_script_url install.sh) | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 $(installer_script_url install.sh) | bash -s -- --install-method git --no-onboard
  OPENCLAW_USE_CN_MIRRORS=1 curl -fsSL --proto '=https' --tlsv1.2 https://clawd.org.cn/install.sh | bash
EOF
}
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL $(installer_script_url install.sh) | bash"
}
...
ensure_pnpm() {
    ...
    if command -v corepack &> /dev/null; then
        ...
        if ! run_quiet_step "Activating pnpm" with_npm_registry_env corepack prepare pnpm@10 --activate; then
    ...
    fix_npm_permissions
    run_quiet_step "Installing pnpm" with_npm_registry_env npm install -g pnpm@10
    refresh_shell_command_cache
    if detect_pnpm_cmd && pnpm_cmd_is_ready; then
        ui_success "pnpm ready ($(pnpm_cmd_pretty))"
        return 0
...
ensure_pnpm_binary_for_scripts() {
    ...
    if command -v corepack >/dev/null 2>&1; then
        ...
        corepack enable >/dev/null 2>&1 || true
        with_npm_registry_env corepack prepare pnpm@10 --activate >/dev/null 2>&1 || true
        refresh_shell_command_cache
        if command -v pnpm >/dev/null 2>&1; then
...
run_pnpm() {
    ...
    with_npm_registry_env "${PNPM_CMD[@]}" "$@"
}
...
main() {
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 $(installer_script_url install.sh) | bash -s -- --install-method npm"
